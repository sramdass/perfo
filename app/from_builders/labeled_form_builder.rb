class LabeledFormBuilder < ActionView::Helpers::FormBuilder
  ...
  %w[text_field text_area password_field collection_select date_select].each do |method_name|
    define_method(method_name) do |name, *args|
      content_tag :div, class: "field" do
        field_label(name, *args) + tag(:br) + super(name, *args) + error_messages(name)
      end
    end
  end
  ...
  def check_box(name, *args)
    content_tag :div, class: "field" do
      super + " " + field_label(name, *args) + error_messages(name)
    end
  end
  ...
  def error_messages(field)
      ...
      content_tag(:span, :class => "inline_error_messages") do
          object.errors[field].map do |msg|
            content_tag(:span, msg) + tag(:br)
          end.join.html_safe
      end
    end
  end
